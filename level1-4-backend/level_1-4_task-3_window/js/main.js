...
function createAddButton(config) {
    ...
    btn.onclick = () => openAddModal(config);
...
function openAddModal(config) {
    ...
    formContent.querySelectorAll("input, select, label, .save-btn").forEach(el => el.remove());
    ...
    config.columns.forEach(col => {
        ...
        inputs.forEach(inputCfg => {
            ...
            label.innerText = (inputCfg.label || col.title) + ": ";
            ...
            formContent.insertBefore(label, formContent.querySelector("#closeModal"));
    ...
    saveBtn.addEventListener("click", () => trySave(config));

    formContent.insertBefore(saveBtn, formContent.querySelector("#closeModal"));

    modal.style.display = "flex";
}

function trySave(config) {
    const modal = document.getElementById("addModal");
